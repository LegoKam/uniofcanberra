...
async function run() {
  ...
  console.log('Publishing all courses from feed...');
  const results = await publishAllCourses();
  ...
  console.log(`Done: ${published.length} published, ${failed.length} failed, ${results.length} total`);
